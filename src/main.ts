...
class App {
  ...
  constructor(private model: tf.GraphModel) {
    ...
    this.miniCanvas.width = inputShape[inputShape.length - 2]
    this.miniCanvas.height = inputShape[inputShape.length - 3]
  ...
  drawMini(): void {
    ...
    const invW = w / window.innerWidth
    const invH = h / window.innerHeight
    for (const points of this.strokes) {
      context.strokeStyle = '#fff'
      context.beginPath()
      points.forEach((pos, i) => {
        if (i === 0)
          context.moveTo(pos.x * invW, pos.y * invH)
        else
          context.lineTo(pos.x * invW, pos.y * invH)
      })
      context.stroke()
    }
  }
  ...
  recognize(): void {
    tf.tidy(() => {
      ...
      const candidates = order.slice(0, 5)
      console.log(candidates.map(i => `${CLASS[i]}: ${probabilities[i] * 100}`).join('\n'))
    })
  }
}
